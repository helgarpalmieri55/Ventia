...
- Produces under `/v1/admin/products`:
  ...
  - **Amendment (review fix wave):** PATCH does NOT auto-dedupe a caller-supplied slug; an explicit slug collision returns 409 `SLUG_TAKEN`. Auto-dedup (`-2`…`-20`) applies only to `create()`'s name-derived slugs. Update runs atomically in one transaction (manual RLS role+GUC escape).
