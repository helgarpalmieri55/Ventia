...
RLS applies to these tables (tenant-owned): `Tenant` is **excluded** (it's the root; scoped client restricts by id), and so are auth tables (`User`, `Session`, `Account`, `Verification`, `Membership`), `WebhookEvent`, `AuditLog` (nullable tenant, system-written). Included: `TenantDomain`, `TenantLimits`, `Category`, `Product`, `ProductCategory`, `ProductVariant`, `ProductImage`, `InventoryMovement`, `Cart`, `CartItem`, `Customer`, `Order`, `OrderItem`, `OrderEvent`, `Payment`, `Conversation`, `Message`, `AgentUsage`, `TenantContent`, `NotificationLog`, `Subscription`, `Shipment`, `Invoice`.
